...
import {
  getEditedOcrTextLines,
  preserveEditedOcrText,
} from "./edited-box-overrides";
import {
  buildMangaPageKey,
  getImageFingerprint,
...
const buildOrderedOcrPages = (pageFiles: string[]) => pageFiles.map((imagePath, pageIndex) => ({
  pageKey: buildMangaPageKey(pageIndex, imagePath),
  imagePath,
}));
...
export async function persistPageResultForManga(
  ...
  return withMangaOcrFileMutationLock(manga.path, async () => {
    ...
    rekeyMangaOcrPagesForMutation(file, buildOrderedOcrPages(pageFiles));
    const pageKey = buildMangaPageKey(pageIndex, imagePath);
    const existingEntry = findOcrPageEntryBySource(file.pages, pageKey, fingerprint)?.entry;
...
export async function readStoredPageFromMangaFile(
  ...
  const pageKey = buildMangaPageKey(pageIndex, imagePath);
  const entry = findOcrPageEntryBySource(file.pages, pageKey, fingerprint)?.entry;
  if (!entry || !isStoredPageUpToDate(entry, { ...fingerprint, imagePath: entry.imagePath }, passProfile)) {
    return null;
  }
...
export async function addManualBoxesToMangaPage(
  ...
  const normalizedResult = await withMangaOcrFileMutationLock(manga.path, async () => {
    const file = await ensureMangaOcrFile(manga, pageFiles.length);
    rekeyMangaOcrPagesForMutation(file, buildOrderedOcrPages(pageFiles));
    const pageKey = buildMangaPageKey(pageIndex, imagePath);
    const existingEntry = findOcrPageEntryBySource(file.pages, pageKey, fingerprint)?.entry;
