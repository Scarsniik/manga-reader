...
Author correspondence also offers an optional advanced pass. It loads the configured number of pages for every retained author through the canonical favorite-author listing engine, merges their manga with the regular multi-source merger, then selects a small batch of the cards backed by the most sources. Each selected card is passed to the canonical manga correspondence engine in author-discovery mode; accepted equivalent sources stay attached to that card even when their localized titles would not merge automatically. Newly discovered author names and direct author-page targets are then sent through the canonical author correspondence engine. A later pass skips every already processed card and takes the next batch.
