...
Foreground and background latest-source scans also use the same optional performance trace format. Reports are disabled by default and can be enabled in the Developer settings tab. Each enabled run records request-limiter waits separately from HTTP execution, plus scheduler rounds, source batches, detail enrichment and listing-page prefetch reuse. Profiles are stored below `data/scraper-latest-diagnostics` and can be summarized with `npm run diagnostics:scraper-latest`.
