...
Author correspondence also offers an optional advanced pass. It loads the configured number of pages for every retained author through the canonical favorite-author listing engine, merges their manga with the regular multi-source merger, then selects a small batch of the cards backed by the most sources. Each selected card is passed through the complete canonical `sameManga` correspondence flow, including its title and author expansion branches, detail enrichment, matching rules and safety guards. Accepted equivalent sources stay attached to that card even when their localized titles would not merge automatically. Newly discovered author names and direct author-page targets are then sent through the canonical author correspondence engine. A later pass skips every already processed card and takes the next batch. The result action lets the user choose the size of each new manga batch independently. The result view publishes the current phase and batch progress, and visually identifies author pages first found by the advanced pass.
...
The author pages and manga sources collected by an advanced author pass use a separate application-session cache owned by the Electron main process. This cache is shared with the search worker and workspace windows, is bounded to the 20 most recently used author jobs, is never included in the persisted background-search payload, and disappears when the application closes. It allows the combined-author view to reuse the pages already read and to display equivalent manga sources found by the advanced pass without another request.
