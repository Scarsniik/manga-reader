...
- Le parametre global `Langue du titre des cartes fusionnees` contient une liste ordonnee et reordonnable par glisser-deposer. Le titre et la couverture affiches viennent ensemble de la premiere source appartenant a la premiere langue prioritaire disponible ; si aucune priorite ne correspond, le premier resultat de la fusion reste utilise.
...
- Si la couverture affichee dans une card multi-search ne charge pas, la card essaie le proxy puis l'URL originale de cette meme couverture avant d'afficher le placeholder. Elle ne bascule pas sur une autre source, afin que l'image reste coherente avec le titre prioritaire.
...
- Le filtre de lancement `Rechercher uniquement les originaux` est transmis au moteur et retire les sources dont l'oeuvre source extraite ne correspond ni au mot-cle commun `original`, ni au mot-cle additionnel `originalSourceKeyword`, avant leur ajout aux resultats. Une card sans nom de source extrait est originale, y compris si seule une URL de source orpheline existe. Le filtre `Originaux uniquement` de la section de resultats reste un filtre d'affichage independant. Une card fusionnee reste visible tant qu'elle conserve au moins une source originale. Un scraper sans detection de source configuree est toujours considere original.
- Les oeuvres sources sont affichees sur les cards sous forme de chips compactes, comme les tags. Un auteur extrait est ajoute aux metadonnees de la card uniquement s'il correspond a un favori auteur du meme scraper.
