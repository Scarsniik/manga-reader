...
- Le filtre de lancement `Rechercher uniquement les originaux` est transmis au moteur et retire les sources dont l'oeuvre source extraite ne correspond pas au mot-cle `originalSourceKeyword` avant leur ajout aux resultats. Le filtre `Originaux uniquement` de la section de resultats reste un filtre d'affichage independant. Une card fusionnee reste visible tant qu'elle conserve au moins une source originale. Un scraper sans detection de source configuree est toujours considere original.
